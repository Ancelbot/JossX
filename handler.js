...
—◉ *Contacto para apelaciones:* wa.me/+56986469687
╚═════════════════════╝
               `.trim();
              m.reply(messageText);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;𝐉𝐨𝐬𝐬 🤺 👑;;;\nFN:𝐉𝐨𝐬𝐬 🤺 👑\nORG:𝐉𝐨𝐬𝐬 🤺 👑\nTITLE:\nitem1.TEL;waid=56986469687:+56 9 8646 9687\nitem1.X-ABLabel:𝐉𝐨𝐬𝐬 🤺 👑\nX-WA-BIZ-DESCRIPTION:[❗] ᴄᴏɴᴛᴀᴄᴛᴀ ᴀ ᴇsᴛᴇ ɴᴜᴍ ᴘᴀʀᴀ ᴄᴏsᴀs ɪᴍᴘᴏʀᴛᴀɴᴛᴇs.\nX-WA-BIZ-NAME:𝐉𝐨𝐬𝐬 🤺 👑\nEND:VCARD`;
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: '𝐉𝐨𝐬𝐬 🤺 👑', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  const msg = {
    rowner: '*[ ℹ️ ] Este comando solo puede ser utilizado por el propietario del 𝐉𝐨𝐬𝐬  🤺.*',
    owner: '*[ ℹ️ ] Este comando solo puede ser utilizado por el propietario del 𝐉𝐨𝐬𝐬  𝐁𝐨𝐭🤺.*',
    mods: '*[ ℹ️ ] Este comando solo puede ser utilizado por moderadores y el propietario del 𝐉𝐨𝐬𝐬  𝐁𝐨𝐭🤺.*',
    premium: '*[ ℹ️ ] Este comando solo puede ser utilizado por usurios premium y el propietario del 𝐉𝐨𝐬𝐬  𝐁𝐨𝐭🤺.*',
    group: '*[ ℹ️ ] Este comando solo puede ser utilizado en grupos.*',
    private: '*[ ℹ️ ] Este comando solo puede ser utilizado en el chat privado del 𝐉𝐨𝐬𝐬  𝐁𝐨𝐭🤺.*',
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ ⚠ ] Advertencia*', body: '𝐉𝐨𝐬𝐬 🤺', thumbnail: imagen1, sourceUrl: 'https://www.instagram.com/ancelzzz/'}}}}, aa);
